...
- (void)commonInit{
    ...
    tabbar_ = [[DLFixedTabbarView alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen]bounds].size.width, self.tabbarHeight)];
    tabbar_.delegate = self;
    [self addSubview:tabbar_];
    
    slideView_ = [[DLSlideView alloc] initWithFrame:CGRectMake(0, self.tabbarHeight+self.tabbarBottomSpacing, [[UIScreen mainScreen]bounds].size.width, self.bounds.size.height-self.tabbarHeight-self.tabbarBottomSpacing)];
